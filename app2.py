...
for msg in msgs[::-1]:
    for response_part in msg:
        if type(response_part) is tuple:
            ...
            # Extracting the reserved (received) date

            received_date = my_msg.get("Date")

            received_datetime = parsedate_to_datetime(received_date)

            input_datetime = datetime.strptime(
                str(received_datetime), "%Y-%m-%d %H:%M:%S%z"
            )
            ...
            received_datetime = converted_datetime.strftime("%Y-%m-%d %H:%M:%S")

            print("reserved_datetime", received_datetime)
            ...
            # received_datetime = str(received_datetime).split("-")[0]
            # received_datetime = received_datetime.split("+")[0]
            c.drawString(20, 660, "Date: {}".format(received_datetime))
            y_position = 620
